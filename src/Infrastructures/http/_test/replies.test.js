...
describe('/threads/{threadId}/comments/{commentId}/replies endpoint', () => {
  ...
  describe('when POST /threads/{threadId}/comments/{commentId}/replies', () => {
    it('should response 401 if payload not access token', async () => {
      ...
      const response = await server.inject({
        method: 'POST',
        url: '/threads/thread-401/comments/comment-401/replies',
        payload: {},
      });
    ...
    it('should response 404 if thread id not valid', async () => {
      ...
      const response = await server.inject({
        method: 'POST',
        url: '/threads/thread-404/comments/comment-404/replies',
        payload: {
    ...
    it('should response 404 if comment id not valid', async () => {
      ...
      const response = await server.inject({
        method: 'POST',
        url: `/threads/${threadResponse.data.addedThread.id}/comments/comment-404/replies`,
        payload: {
          content: 'lorem ipsum dolor sit amet',
        },
        headers: { Authorization: `Bearer ${responseAuth.data.accessToken}` },
  ...
  describe('when DELETE /threads/{threadId}/comments/{}/replies/{id}', () => {
    ...
    it('should response 404 if reply not found', async () => {
      ...
      const response = await server.inject({
        method: 'DELETE',
        url: `/threads/${threadResponse.data.addedThread.id}/comments/${commentResponse.data.addedComment.id}/replies/reply-404`,
        headers: { Authorization: `Bearer ${responseAuth.data.accessToken}` },
      });
      ...
      expect(response.statusCode).toEqual(404);
      expect(responseJson.status).toEqual('fail');
      expect(responseJson.message).toEqual('No replies found');
    });
